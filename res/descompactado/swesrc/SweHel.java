...
public class SweHel implements java.io.Serializable {
...
#ifdef TRACE0
    Trace.log("SweHel(SwissEph, SwissLib, Swemmoon, SweDate, SwissData)");
#endif /* TRACE0 */
    this.sw=sw;
    this.sl=sl;
#ifndef ASTROLOGY
  ...
  */
  private double OpticFactor(double Bback,
    ...
    System.err.println("Fa =" + Fa + "\n");
    System.err.println("Fm =" + Fm + "\n");
    System.err.println("Fsc=" + Fsc + "\n");
    System.err.println("Fci=" + Fci + "\n");
    System.err.println("Fcb=" + Fcb + "\n");
...
#if 0
int call_swe_calc(double tjd, int ipl, int iflag, double *x, char *serr) 
{
  int retval = SweConst.OK, ipli, i;
  double dtjd;
  static double tjdsv[3];
  static double xsv[3][6];
  static int iflagsv[3];
  ipli = ipl;
  if (ipli > SweConst.SE_MOON) 
    ipli = 2;
  dtjd = tjd - tjdsv[ipli];
  if (tjdsv[ipli] != 0 && iflag == iflagsv[ipli] && SMath.abs(dtjd) < 5.0 / 1440.0) {
    for (i = 0; i < 3; i++) 
      x[i] = xsv[ipli][i] + dtjd * xsv[ipli][i+3];
    for (i = 3; i < 6; i++) 
      x[i] = xsv[ipli][i];
  } else {
    retval = sw.swe_calc(tjd, ipl, iflag, x, serr);
    tjdsv[ipli] = tjd;
    iflagsv[ipli] = iflag;
    for (i = 0; i < 6; i++) 
      xsv[ipli][i] = x[i];
...
StringBuffer call_swe_fixstar_mag__star_save = new StringBuffer();
  ...
  int call_swe_rise_trans(double tjd, int ipl, String star, int helflag, int eventtype, double[] dgeo, double atpress, double attemp, double[] tret, StringBuffer serr) {
    int retval;
#ifdef NO_JPL
    int iflag = helflag & (SweConst.SEFLG_SWIEPH|SweConst.SEFLG_MOSEPH);
#else
...
#endif /* NO_JPL */
      int iflag = epheflag | SweConst.SEFLG_EQUATORIAL;
      iflag |= SweConst.SEFLG_NONUT | SweConst.SEFLG_TRUEPOS;
      tjd_tt = JDNDaysUT + SweDate.getDeltaT(JDNDaysUT);
      if (sw.swe_calc(tjd_tt, SweConst.SE_SUN, iflag, x, serr) != SweConst.ERR) {
        sunRA_ralast = x[0];
...
#if 0
private int fast_magnitude(double tjd, double[] dgeo, StringBuffer ObjectName, int helflag, double *dmag, StringBuffer serr)
{
  int retval = SweConst.OK, ipl, ipli;
  double dtjd;
  static double tjdsv[]=new double[3];
  static double dmagsv[]=new double[3];
  static int helflagsv[3];
  ipl = DeterObject(ObjectName);
  ipli = ipl;
  if (ipli > SweConst.SE_MOON) 
    ipli = 2;
  dtjd = tjd - tjdsv[ipli];
  if (tjdsv[ipli] != 0 && helflag == helflagsv[ipli] && SMath.abs(dtjd) < 5.0 / 1440.0) {
    *dmag = dmagsv[ipli];
  } else {
    retval = Magnitude(tjd, dgeo, ObjectName, helflag, dmag, 0, serr);
    tjdsv[ipli] = tjd;
  ...
  */
  private double VisLimMagn(double[] dobs, double AltO, double AziO, double AltM, double AziM, double JDNDaysUT, double AltS, double AziS, double sunra, double Lat, double HeightEye, double[] datm, int helflag, int[] scotopic_flag, StringBuffer serr) {
    ...
    if (SN <= 0.00000001)
      SN = 0.00000001;
    return -16.57 - 2.5 * (SMath.log(Th) / log10) - kX + 5.0 * (SMath.log(SN) / log10);*/
#endif
...
#if 0
int FAR PASCAL_CONV HeliacalJDut(double JDNDaysUTStart, double Age, double SN, double Lat, double Longitude, double HeightEye, double Temperature, double Pressure, double RH, double VR, char *ObjectName, int TypeEvent, char *AVkind, double *dret, StringBuffer serr)
{
  double dgeo[]=new double[3], datm[]=new double[4], dobs[]=new double[6];
  int helflag = SweConst.SE_HELFLAG_HIGH_PRECISION;
  helflag |= SweConst.SE_HELFLAG_AVKIND_VR;
  dgeo[0] = Longitude;
  dgeo[1] = Lat;
  dgeo[2] = HeightEye;
  datm[0] = Pressure;
...
#if 0
  double AltM, AziM;
        if ((retval = sw.swe_calc(tjd_tt, SweConst.SE_MOON, iflag, x, serr)) == SweConst.ERR)
//          goto swe_heliacal_err;
          return goto_swe_heliacal_err1(serr, serr_ret, retval);
        xin[0] = x[0];
        xin[1] = x[1];
...
//    goto swe_heliacal_err;
  ...
  if ((helflag & SweConst.SE_HELFLAG_AVKIND_VR) != 0) {
    ...
    if (swe_heliacal_pheno_ut(JDNarcvisUT, dgeo, datm, dobs, ObjectName, TypeEvent, helflag, darr, serr) != SweConst.OK)
      return SweConst.ERR;
    JDNarcvisUT = darr[13];
    }
  }
...
//    goto swe_heliacal_err;
  ...
  private int get_asc_obl_old(double tjd, int ipl, StringBuffer star, int iflag, double[] dgeo, boolean desc_obl, double[] daop, StringBuffer serr) {
    ...
    String s;
    if (star != null && star.length() != 0) {
      if ((retval = call_swe_fixstar(star, tjd, epheflag | SweConst.SEFLG_EQUATORIAL, x, serr)) == SweConst.ERR)
        return SweConst.ERR;
    } else {
      if ((retval = sw.swe_calc(tjd, ipl, epheflag | SweConst.SEFLG_EQUATORIAL, x, serr)) == SweConst.ERR)
        return SweConst.ERR;
  ...
  private int get_asc_obl_with_sun_old(double tjd_start, int ipl, StringBuffer star, int helflag, int TypeEvent, double[] dgeo, double[] tjdret, StringBuffer serr) {
    ...
    int epheflag = helflag & (SweConst.SEFLG_JPLEPH|SweConst.SEFLG_SWIEPH|SweConst.SEFLG_MOSEPH);
#endif /* NO_JPL */
    double dsunpl[] = new double[]{1}, tjd, daystep, dsunpl_save;
    double dsynperiod = 367;
    double dangsearch = 0;
    boolean desc_obl = false;
    ...
    if (retval != SweConst.OK)  /* retval may be SweConst.ERR or -2 */
      return retval;
    while (dsunpl[0] < 359.99999) {
      dsunpl_save = dsunpl[0];
      daystep /= 2.0;
      retval = get_asc_obl_diff(tjd + daystep, ipl, star, epheflag, dgeo, desc_obl, dsunpl, serr);
      if (retval != SweConst.OK)  /* retval may be SweConst.ERR or -2 */
        return retval;
      if (dsunpl[0] > dsunpl_save)
        tjd += daystep;
      else
        dsunpl[0] = dsunpl_save;
    }
    tjdret[0] = tjd;
    return SweConst.OK;
  }
  ...
  private int get_asc_obl_acronychal(double tjd_start, int ipl, StringBuffer star, int helflag, int TypeEvent, double[] dgeo, double[] tjdret, StringBuffer serr) {
    ...
    double dsynperiod = 367;
    double aosun[] = new double[1], aopl[] = new double[1];
    boolean sun_desc = true, obj_desc = false;
    daystep = dsynperiod;
    tjd = tjd_start;
    if (TypeEvent == 4) {
      sun_desc = false; 
    ...
    retval = get_asc_obl(tjd, SweConst.SE_SUN, "", epheflag, dgeo, sun_desc, aosun, serr);
    if (retval != SweConst.OK)  /* retval may be SweConst.ERR or -2 */
      return retval;
    ...
    if (retval != SweConst.OK)  /* retval may be SweConst.ERR or -2 */
      return retval;
    dsunpl = sl.swe_degnorm(aosun[0] - aopl[0] + 180);
    while (dsunpl < 359.99999) {
      dsunpl_save = dsunpl;
      daystep /= 2.0;
      /* ascensio (descensio) obliqua of sun */
      retval = get_asc_obl(tjd+daystep, SweConst.SE_SUN, "", epheflag, dgeo, sun_desc, aosun, serr);
      if (retval != SweConst.OK)  /* retval may be SweConst.ERR or -2 */
        return retval;
      /* ascensio (descensio) obliqua of body */
...
#if 0
private int get_acronychal_day_new(double tjd, double[] dgeo, double[] datm, double[] dobs, char *ObjectName, int helflag, int TypeEvent, double *thel, StringBuffer serr) {
  double tjdc = tjd, tret, x[]=new double[6], xaz[]=new double[6], AltO = -10;
  ...
  int epheflag = helflag & (SweConst.SEFLG_JPLEPH|SweConst.SEFLG_SWIEPH|SweConst.SEFLG_MOSEPH);
#endif /* NO_JPL */
  int iflag = epheflag | SweConst.SEFLG_EQUATORIAL | SweConst.SEFLG_TOPOCTR;
  if ((retval = my_rise_trans(tret, 0, ObjectName, SweConst.SE_CALC_RISE, helflag, dgeo, datm, &tret, serr)) == SweConst.ERR) return SweConst.ERR;
  trise = tret;
  tret += 0.01
  if ((retval = my_rise_trans(tret, 0, ObjectName, SweConst.SE_CALC_SET, helflag, dgeo, datm, &tret, serr)) == SweConst.ERR) return SweConst.ERR;
  trise = tset;
  ...
  *thel = tret;
  ...
  private int get_acronychal_day_old(double tjd, double[] dgeo, double[] datm, double[] dobs, StringBuffer ObjectName, int helflag, int TypeEvent, double[] thel, StringBuffer serr) {
    ...
    while (AltO < 0) {
      ...
      if (iter_day == -1)
        tret[0] -= 1;
      retval = my_rise_trans(tret[0], SweConst.SE_SUN, "", is_rise_or_set, helflag, dgeo, datm, tret, serr);
      if (retval != SweConst.OK)
        return retval;
      /* determine object's position */
      if (ipl == -1)
        retval = call_swe_fixstar(ObjectName, tret[0]+SweDate.getDeltaT(tret[0]), iflag, x, serr);
      else 
        retval = sw.swe_calc(tret[0]+SweDate.getDeltaT(tret[0]), ipl, iflag, x, serr);
      if (retval == SweConst.ERR) return SweConst.ERR;
      sc.swe_azalt(tret[0], SweConst.SE_EQU2HOR, dgeo, datm[0], datm[1], x, xaz);
      AltO = xaz[2];
    }
    thel[0] = tret[0];
    return SweConst.OK;
